...
EOF
    ...
    for f in data/train/feats.scp exp/tri2_ali/ali.1.gz exp/tri2/final.mdl; do
...
if [ $stage -le 1 ]; then
    ...
    echo "-- $0: $(date): Get the alignments as lattices --"
    steps/align_fmllr_lats.sh --nj $nj --cmd $cmd data/train \
      data/lang exp/tri2 $nn_latdir
    
    rm ${nn_latdir}/fsts.*.gz # save space
    ...
    steps/nnet3/chain/build_tree.sh \
        ...
        $lang_train exp/tri2_ali $nn_treedir
fi
...
if [ $stage -le 3 ]; then
    ...
    steps/nnet3/chain/train.py \
        ...
        --egs.opts "--num_egs_diagnostic 10 --num_utts_subset 40 --frames-overlap-per-eg 0" \
