...
# exp/chain/cnn1a/: num-iters=153 nj=3..10 num-params=3.6M dim=$FEAT_DIM->268 combine=-0.034->-0.034 xent:train/valid[101,152,final]=(-0.097,-0.186,-0.092/-0.101,-0.212,-0.098) logprob:train/valid[101,152,final]=(-0.035,-0.067,-0.035/-0.036,-0.082,-0.035)
...
chunk_width=16,8  # kitt: to deal with the memory allocation problem
...
if [ $stage -le 4 ]; then
  ...
  conv-relu-batchnorm-layer name=cnn1 height-in=$FEAT_DIM height-out=$FEAT_DIM time-offsets=-2,-1,0,1,2 $common1
  conv-relu-batchnorm-layer name=cnn2 height-in=$FEAT_DIM height-out=$FEAT_DIM time-offsets=-2,-1,0,1,2 $common1
