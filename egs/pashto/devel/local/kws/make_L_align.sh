...
if [ $# -ne 3 ]; then
  ...
  echo "utterances"
  echo "The script automatically recognizes the probabilistic lexicon"
...
if  [ -f ${tmpdir}/lexicon.txt ] ; then
  ...
  cat ${tmpdir}/lexiconp.txt | \
    awk '{printf("%s #0 ", $1); for (n=3; n <= NF; n++) { printf("%s ", $n); } print "#1"; }' | \
    utils/make_lexicon_fst.pl - 0.5 $silphone | \
    fstcompile --isymbols=${dir}/phones.txt --osymbols=${dir}/words.txt \
    --keep_isymbols=false --keep_osymbols=false | \
    fstarcsort --sort_type=olabel > ${outdir}/L_align.fst
else
  echo >&2 "ERR: Neither ${tmpdir}/lexicon.txt nor ${tmpdir}/lexiconp.txt exists."
  exit 1
fi
exit 0;
