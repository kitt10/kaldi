...
#
#
# This script trains an LM on the UW3 training transcriptions -- ADJUSTED FOR PASHTO (kitt 10/2018).
...
if [ $stage -le 0 ]; then
  ...
  tail -n +$[$num_dev_sentences+1] < data/train/text | cut -d " " -f 2- >  ${dir}/data/text/pashto.txt

  # for reporting perplexities, we'll use the "real" dev set.
  # (a subset of the training data is used as ${dir}/data/text/pashto.txt to work
...
if [ $stage -le 1 ]; then
  ...
  train_lm.py  --wordlist=${wordlist} --num-splits=10 --warm-start-ratio=20  \
               ...
               --fold-dev-into=pashto ${bypass_metaparam_optim_opt} \
