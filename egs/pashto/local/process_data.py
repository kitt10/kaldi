...
def parse_args():
    ...
    parser.add_argument('--n_samples', type=int, help='number of samples per speaker to consider')
    parser.add_argument('--invert', type=bool, default=False, help='invert colors (black/white) for all images')
    ...
    for w_id in listdir(args.data_path_im):
        with cod_open(join_path(args.data_path_tr, w_id+'.txt'), 'r', encoding='utf-8') as f:
            for line in f.readlines():
                ...
                spk_id = im_orig_filename.split('_')[1]
                ...
                # copy the image (and invert colors if wanted)
                if args.invert:
                    ImageOps.invert(Image.open(im_orig_path)).save(im_path)
                else:
                    copyfile(im_orig_path, im_path)
