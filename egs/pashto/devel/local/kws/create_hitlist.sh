...
wbegin=$(grep "#0" $lang/phones.txt | head -1 | awk '{print $2}')
wend=$(grep "#1" $lang/phones.txt | head -1 | awk '{print $2}')

if [ -z $wbegin ] || [ -z $wend ]; then
  echo "ERR: Check ${lang}/phones.txt, no #0, #1" && exit 1;
fi

echo "-- Generating ${lang}/L_align.fst"
local/kws/make_L_align.sh $lang_tmp $lang $lang 2>&1 | tee ${dir}/log/L_align.log
echo "-- L_align.fst generated."

echo "-- Generating hitlist"
